        ...
        if args.checkpoint_path:
            ...
            while aggressive_flag:
                ...
                for i, train_batch in tk0:
                    ...
                    trainer.fit(
                    model, train_dataloaders=train_enc_dataloader    
                    )    
                    model.unfreeze_decoder()
                    
                    #decoder
                    model.freeze_encoder()
                    train_dec_dataloader = DataLoader(
                    train_batch, batch_size=batch_size, num_workers=cpu_count,
                    )
                    trainer = pl.Trainer(
                        gpus=-1,
                        accelerator="ddp",
                    ...
                    trainer.fit(
                    model, train_dataloaders=train_dec_dataloader    
                    )
