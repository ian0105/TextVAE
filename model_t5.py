...
class T5VAE(LightningModule):
    ...
    def freeze_encoder(self):
        for param in self.t5.encoder.parameters():
            param.requires_grad = False
        for param in self.t5.mu.parameters():
            param.requires_grad = False
        for param in self.t5.logvar.parameters():
            param.requires_grad = False
    ...
    def calc_mi_q(self,x):
        ...
        # [x_batch, nz]
        encoder_outputs = self.t5.run_encoder(input_ids=x,output_hidden_states=True)
        pooled = self.t5.pool(encoder_outputs.hidden_states)
        z, mu, logvar = self.t5.calculate_latent(pooled)
        z = z.unsqueeze(1)
        x_batch, nz = mu.size()
    ...
    def run_batch(self, batch, batch_idx, training=False):
        ...
        x, z, mu, logvar = self(
            encoder_inputs.to('cuda'),
            encoder_masks.to('cuda'),
            labels=decoder_targets.to('cuda'),
        )

        recon_loss = self.reconstruction_loss(x, decoder_targets.to('cuda'))
    ...
    def training_one_step(self, batch, batch_idx, device):
