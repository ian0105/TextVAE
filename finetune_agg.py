...
def calc_mi(model, dataloader):
    ...
    for batch_data in dataloader:
        batch_data = batch_data[0].to('cuda')
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        if args.checkpoint_path:
            ...
            train_loss = report_kl_loss = report_rec_loss = 0
            epoch = 0
            while aggressive_flag:
                print("\nBurning Mode!!\n")
                tk0 = tqdm(train_dataloader,total=len(train_dataloader))
                for i, train_batch in enumerate(tk0):
                    #encoder
                    ...
                    for sub_iter, train_enc_batch in enumerate(train_enc_dataloader):
                        optimizer.zero_grad()
                        loss,_,_ = model.training_one_step(train_enc_batch,sub_iter,device)
                        burn_cur_loss += loss
                        loss.backward()
                        optimizer.step()
                        if sub_iter % 15 == 0:
                    ...
                    optimizer.zero_grad()
                    loss,recon_loss,reg_loss = model.training_one_step(train_batch,i,device)
                    loss.backward()
                    optimizer.step()
                    ...
                    if i % (batch_size*10) == 0:
                        ...
                        model.train()
                        print('epoch: %d, iter: %d, total_loss: %.4f, kl: %.4f, mi: %.4f, recon: %.4f,' \
                               'time elapsed %.2fs' %
                               (epoch, i, train_loss, report_kl_loss, mi,
                               report_rec_loss, time.time() - start))
                        report_kl_loss = report_rec_loss = train_loss = 0
        ...
        torch.save(model.state_dict(), 'agg_finish.pt')            
